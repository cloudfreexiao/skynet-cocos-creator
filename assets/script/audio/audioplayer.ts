...
import { gen_handler} from "../utils/handler"
import * as Consts from "../consts/consts"
import * as wxapi from "../wxapi/wxstorage"
// import Map from "../utils/tsmap"
...
export class AudioPlayer
{
    ...
    play_music(name:string)
    {
        ...
        let path = strfmt(MUSIC_PATH, name);
    ...
    {
        let aid = cc.audioEngine.play(clip.toString(), loop, volume); // play 第一個參數為string
    ...
    play_sound(name:string)
    {
        ...
        let path = strfmt(SOUND_PATH, name);
        let clip = this.clip_cache.get(path);
        if(clip)
        {
